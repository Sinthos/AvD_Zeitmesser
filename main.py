...
class LightBarrierApp:
    def __init__(self, root):
        ...
        # Tabelle erstellen
        self.tree = ttk.Treeview(root, columns=('Messungsname', 'Startzeit', 'Endzeit', 'Dauer', 'Strafzeit'), show='headings')
    ...
    def set_start_time(self):
        ...
        item_id = self.tree.insert('', 'end', values=('Unbenannt', start_time, 'Läuft...', '0.00 s', '0'))
    ...
    def set_end_time(self):
        if self.timer_queue:
            ...
            if item_id in self.timer_items:
                ...
                self.tree.item(item_id, values=('Unbenannt', start_time, end_time, '{:.2f} s'.format(duration), '0'))
    ...
    def update_timer(self):
        ...
        for item_id, (start_time, _) in list(self.timer_items.items()):
            ...
            self.tree.item(item_id, values=('Unbenannt', start_time, 'Läuft...', '{:.2f} s'.format(duration), '0'))
    ...
    def export_to_excel(self):
        ...
        if filename:
            ...
            for i, item_id in enumerate(self.tree.get_children(), start=1):
                messungsname, start_time, end_time, duration, strafzeit = self.tree.item(item_id)['values']
